...
bool Lexer::isOp(uint32_t first, uint32_t second, bool &two_char, OpRec &op, std::string full_value)
{
    if (full_value == "or")
    {
    ...
    while (true)
    {
        ...
        else if (val == '\\' && not_raw)
        {
            ...
            else if (val2 == 'b')
            {
                ...
                for (int loopc = 0; loopc < 8; loopc++)
                {
                    ...
                    else
                    {
                        ...
                        addError(Error(&current_token, "Not a binary digit", buf));
                    }
                }
            ...
            else
            {
                ...
                addError(Error(&current_token, "Unknown escape sequence", buf));
            }
        }
        else if (val == '\n')
        {
            endToken(ch);
            addError(Error(&current_token, "Unterminated string literal"));
            return;
        }
        else
        {
...
void Lexer::readNumber()
{
    ...
    while (true)
    {
        ...
        else if ((ch.val >= 'a' && ch.val <= 'f') || (ch.val >= 'A' && ch.val <= 'F'))
        {
            if (!hex_prefixed)
            {
        ...
        else if (ch.val == '.')
        {
            ...
            {
                endToken(ch);
                addError(Error(&current_token, "Float has more than one dot"));
                return;
            }
            else
            {
    ...
    while (more)
    {
        ...
        while (true)
        {
                ...
                else if (begin.col < occ)
                {
                    while (indentations.size() > 1)
                    {
                        ...
                        else if (begin.col > indentations.top())
                        {
                            ...
                            addError(Error(&dummy, "Mismatched indent levels", buf));
                            push(begin);
                            break;
                        }
                        else
            ...
            else
            {
                ...
                addError(Error(&dummy, "Unknown character", buf));
            }
        }
    }
}
