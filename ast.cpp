...
int Parser::getPrecedence()
{
    ...
    if (!lexer->isOp(current.value[0], current.value.size() == 1 ? 0 : current.value[1], dummy, rec,
                     current.toString()))
    {
        return -1;
    }
...
Expr *Parser::parsePrimary()
{
    ...
    else
    {
        ...
        addError(Error(&current, "Unknown token type parsing primary expression", buf));
        return 0;
    }
}
...
Expr *Parser::parseImport(Token t)
{
    ...
    {
        addError(Error(&t, "Import specified but image type '" + configuration->image->name() +
...
Expr *Parser::parseDef()
{
    ...
    if (parsing_imports && (is_macro || is_extern || is_generic))
    {
        ...
        addError(Error(&current, thing + " functions are not permitted in interfaces"));
    }
    ...
    FunctionScope *fs = new FunctionScope(current_scope, name, ft);
    ...
    DefExpr *ret = new DefExpr(ft, fs, is_macro, is_extern, is_generic, v, name, lib);
        ...
        if (current.type == CLOSE_BRACKET)
        {
            ...
            else
            {
                ...
                if (t)
                {
                    ...
                    if (current.type != EOL)
                    {
                        addError(Error(&current, "Expected EOL after return type"));
                        expectedEol();
                    }
                    else
                    {
...
Expr *Parser::parseFptr()
{
    ...
    FunctionType *ft = is_extern ? new ExternalFunctionType(calling_convention) : new FunctionType(false);
...
void Parser::checkInterfaceTypes(Token &current, std::string &name, FunctionScope *prev, std::vector<Type *> &ftypes,
                                 Type *ret)
{
    ...
    {
        char buf[4096];
        sprintf(buf, "Interface for function %s has %ld arguments, implementation %ld", name.c_str(), ftypes.size(),
                prevt->getParams().size());
    ...
    {
        char buf[4096];
        sprintf(buf, "Interface for function %s has %s return type, implementation %s", name.c_str(),
...
{
    indent(i);
    fprintf(log_file, "Vardef type %s name %s %s", value->type->name().c_str(), value->name.c_str(),
            is_const ? "const" : "");
    if (assigned)
    {
        fprintf(log_file, " = ");
...
Value *VarDefExpr::codegen(Codegen *c)
{
    ...
    if (!value->type->construct(c, value, r))
    {
        printf("Can't construct! Type %s to %s\n", value->type->name().c_str(),
               r->type ? r->type->name().c_str() : "<number>");
    }
    return 0;
}
...
Value *BinaryExpr::codegen(Codegen *c)
{
    ...
    if (token.toString() == "and")
    {
        ...
        c->block()->add(Insn(SELEQ, cmp1, Operand::usigc(0), Operand::usigc(LOGICAL_TRUE)));
        c->block()->add(Insn(CMP, rh, Operand::usigc(0)));
        c->block()->add(Insn(SELEQ, cmp2, Operand::usigc(0), Operand::usigc(LOGICAL_TRUE)));
    ...
    else if (token.toString() == "or")
    {
        ...
        c->block()->add(Insn(SELEQ, v, Operand::usigc(0), Operand::usigc(LOGICAL_TRUE)));
        return v;
    }
    ...
    else if (op == '+')
    {
        ...
        if (incr > 1)
        {
            Value *mul = c->getTemporary(rh->type ? rh->type : register_type, "ptrmul");
    ...
    else if (op == toOp("+=") || op == toOp("-=") || op == toOp("*=") || op == toOp("/=") || op == toOp("&=") ||
             op == toOp("|=") || op == toOp("^="))
    {
...
Value *UnaryExpr::codegen(Codegen *c)
{
    ...
    if (token.toString() == "not")
    {
        ...
        c->block()->add(Insn(SELEQ, v, Operand::usigc(LOGICAL_TRUE), Operand::usigc(0)));
        return v;
    }
    else if (op == '@')
    {
...
Value *Yield::codegen(Codegen *c)
{
    if (ret)
    {
        ...
        if (!returnvar_type->canActivate() && to_ret_type && to_ret_type->canActivate())
        {
    ...
    c->block()->add(Insn(LOAD, Operand::reg(assembler->framePointer()), Operand::reg(assembler->framePointer())));
    c->block()->add(
        Insn(LOAD, Operand::reg(0), Operand::reg(assembler->framePointer()), Operand::sigc(assembler->ipOffset())));
...
void VarRefExpr::store(Codegen *c, Value *v)
{
    ...
    if (i->type->inRegister() && copied->type->inRegister() && elements.size() == 0 && depth == 0)
    {
    ...
    if (!etype->canCopy(copied->type))
    {
        ...
        fprintf(log_file, "Don't know how to copy value of type %s to type %s!\n",
                copied->type ? copied->type->name().c_str() : "constant", etype->name().c_str());
        return;
    }
...
Value *Funcall::codegen(Codegen *c)
{
    ...
    if (ptr->type->isMacro())
    {
        addError(Error(&token, "Can't call macros directly", name()));
        return 0;
    }
    else if (c->getScope()->getType()->isMacro() && (ptr->type->isMacro() == false))
    {
        addError(Error(&token, "Can't call normal code from macro", name()));
        return 0;
    }
    ...
    else
    {
        ...
        c->block()->add(
            Insn(LOAD, sl, Operand::reg(assembler->framePointer()), Operand::sigc(assembler->staticLinkOffset())));
        int count = depth - 1;
        if (is_import_call)
        {
            count++;
        ...
        {
            c->block()->add(Insn(LOAD, sl, sl, Operand::sigc(assembler->staticLinkOffset())));
        }
    }
