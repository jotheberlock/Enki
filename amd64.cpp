...
const char *regnames[] = {"rax", "rcx", "rdx", "rbx", "rsp", "rbp", "rsi", "rdi", "r8",
...
bool Amd64::assemble(BasicBlock *b, BasicBlock *next, Image *image)
{
    ...
    for (auto &i : code)
    {
        ...
        case STORE16: {
            ...
            *current++ = (0x40 | rexreg(i.ops[sr].getReg(), i.ops[dr].getReg()));
        ...
        case LOADS16: {
            ...
            *current++ = (0x48 | rexreg(i.ops[sr].getReg(), i.ops[dr].getReg()));
            *current++ = 0x0f;
        ...
        case LOADS32: {
            ...
            unsigned char do64 =
                (i.ins == LOAD || i.ins == LOAD64 || i.ins == STORE || i.ins == STORE64 || i.ins == LOADS32) ? 0x8
                                                                                                             : 0x0;

            *current++ = (0x40 | do64 | rexreg(i.ops[sr].getReg(), i.ops[dr].getReg()));
        ...
        case MOVE: {
                ...
                i.ops[1].isSection() || i.ops[1].isExtFunction())
            {
                ...
                *current++ = rex;
                ...
                if (i.ops[1].isFunction())
                {
                    uint64_t reloc = 0xdeadbeefdeadbeefLL;
            ...
            else
            {
                ...
                *current++ = 0x8b;
                unsigned char rr = (0x3 << 6) | (reg(i.ops[0].getReg() & 0x7) << 3) // dest
        ...
        case XOR: {
                ...
                *current++ = 0x81;
                ...
                case XOR:
                    *current++ = 0xf0 | rr;
                    break;
                default:
                    fprintf(log_file, "Unknown logop %ld!\n", i.ins);
                    break;
                }
            ...
            else
            {
                *current++ = 0x48 | rexreg(i.ops[2].getReg(), i.ops[0].getReg());
                ...
                unsigned char rr = 0xc0 | (reg(i.ops[2].getReg() & 0x7) << 3) // src
        ...
        case CMP: {
            ...
            if (i.ops[1].isReg())
            {
                *current++ = (0x48 | rexreg(i.ops[1].getReg(), i.ops[0].getReg()));
                *current++ = 0x39;
        ...
        case REMS: {
            ...
            *current++ = 0xf7;
            ...
            if (i.ins == REM || i.ins == REMS)
            {
                ...
                *current++ = 0x48;
                *current++ = 0x8b;
                unsigned char rr = (0x3 << 6) | (reg(0 & 0x7) << 3) // dest
                                   | reg(2 & 0x7);                  // src
                *current++ = rr;
            }
        ...
        case SELGTS: {
            ...
            *current++ = 0x0f;
            *current++ = op1;
            unsigned char rr = 0xc0 | (i.ops[1].getReg() & 0x7) | ((i.ops[0].getReg() & 0x7) << 3);
            *current++ = rr;
            ...
            *current++ = 0x0f;
            *current++ = op2;
            rr = 0xc0 | (i.ops[2].getReg() & 0x7) | ((i.ops[0].getReg() & 0x7) << 3);
            *current++ = rr;
            break;
        }
        default: {
...
void Amd64WindowsCallingConvention::generatePrologue(BasicBlock *b, FunctionScope *f)
{
    ...
    for (unsigned int loopc = 1; loopc < args.size(); loopc++)
    {
        ...
        {
            b->add(Insn(LOAD, Operand(args[loopc]), Operand::reg("rsp"), Operand::sigc(32 + (count - 4) * 8)));
        }
        count++;
    }
...
Value *Amd64WindowsCallingConvention::generateCall(Codegen *c, Value *fptr, std::vector<Value *> &args)
{
    ...
    current->add(Insn(SUB, Operand::reg("rsp"), Operand::reg("rsp"), Operand::usigc(stack_size)));
    ...
    for (unsigned int loopc = 0; loopc < args.size(); loopc++)
    {
        ...
        else
        {
            ...
            current->add(Insn(STORE, Operand::reg("rsp"), Operand::sigc(offs), Operand(args[loopc])));
    ...
    current->add(Insn(ADD, Operand::reg("rsp"), Operand::reg("rsp"), Operand::usigc(stack_size)));
...
Value *Amd64UnixCallingConvention::generateCall(Codegen *c, Value *fptr, std::vector<Value *> &args)
{
    BasicBlock *current = c->block();
    RegSet res;
    ...
    for (unsigned int loopc = 0; loopc < args.size(); loopc++)
    {
        ...
        {
            current->add(
                Insn(STORE, Operand::reg("rsp"), Operand::sigc(((loopc - 6) * 8) - stack_size), Operand(args[loopc])));
            continue;
        }
...
Value *Amd64UnixSyscallCallingConvention::generateCall(Codegen *c, Value *, std::vector<Value *> &args)
{
    BasicBlock *current = c->block();
    RegSet res;
