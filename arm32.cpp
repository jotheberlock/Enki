...
bool Arm32::assemble(BasicBlock *b, BasicBlock *next, Image *image)
{
    ...
    for (auto &i : code)
    {
        ...
        case LOAD32: {
            ...
            {
                mc = 0xe5100000 | sign_bit | (uval & 0xfff) | i.ops[0].getReg() << 12 | i.ops[1].getReg() << 16;
            }
            else if (i.ins == LOAD8)
            {
                mc = 0xe5d00000 | sign_bit | (uval & 0xfff) | i.ops[0].getReg() << 12 | i.ops[1].getReg() << 16;
            }
            else if (i.ins == LOAD16)
            {
                mc = 0xe15000b0 | sign_bit | (uval & 0xf) | (uval & 0xf0 << 4) | i.ops[0].getReg() << 12 ||
                     i.ops[1].getReg() << 16;
            }
            else if (i.ins == LOADS8)
            {
                mc = 0xe15000d0 | sign_bit | (uval & 0xf) | (uval & 0xf0 << 4) | i.ops[0].getReg() << 12 ||
                     i.ops[1].getReg() << 16;
            }
            else if (i.ins == LOADS16)
            {
                mc = 0xe15000f0 | sign_bit | (uval & 0xf) | (uval & 0xf0 << 4) | i.ops[0].getReg() << 12 ||
                     i.ops[1].getReg() << 16;
            }
            break;
        }
        ...
        case STORE32: {
            ...
            {
                mc = 0xe5000000 | sign_bit | (uval & 0xfff) | i.ops[dest].getReg() << 12 | i.ops[0].getReg() << 16;
            }
            else if (i.ins == STORE16)
            {
                mc = 0xe14000b0 | sign_bit | (uval & 0xf) | (uval & 0xf0 << 4) | i.ops[dest].getReg() << 12 ||
                     i.ops[0].getReg() << 16;
            }
            else if (i.ins == STORE8)
            {
                mc = 0xe5400000 | sign_bit | (uval & 0xfff) | i.ops[dest].getReg() << 12 | i.ops[0].getReg() << 16;
            }
        ...
        case MOVE: {
                ...
                i.ops[1].isSection() || i.ops[1].isExtFunction())
            {
                ...
                if (br)
                {
                    br->addReloc(0, 0, 0xfff, 0, 32);
                    br->addReloc(0, 12, 0xf, 16, 32);
                    br->addReloc(4, 16, 0xfff, 0, 32);
                    br->addReloc(4, 28, 0xf, 16, 32);
                }
            ...
            {
                mc = 0xe1a00000 | (i.ops[1].getReg()) | i.ops[0].getReg() << 12;
            }
        ...
        case ROR: {
            ...
            uint32_t op = 0;
            uint32_t const_shift = (i.ops[2].isUsigc()) ? (uint32_t)i.ops[2].getUsigc() : 0;
        ...
        case NOT: {
            ...
            assert(i.ops[0].isReg());
            assert(i.ops[1].isReg());
            mc = 0xe1e00000 | (i.ops[0].getReg() << 12) | (i.ops[1].getReg() << 16);
            break;
        }
        case SELEQ:
        case SELGE:
        ...
        case SELGTS: {
            ...
            mc = op1 | 0x01a00000 | (i.ops[1].getReg()) | i.ops[0].getReg() << 12;
            wee32(le, current, mc);
            mc = op2 | 0x01a00000 | (i.ops[2].getReg()) | i.ops[0].getReg() << 12;
            break;
        }
        case BRA: {
            assert(i.oc == 1);
            ...
            else
            {
                ...
                BasicBlockRelocation *bbr =
                    new BasicBlockRelocation(image, current_function, flen(), flen() + 8, i.ops[0].getBlock());
                bbr->addReloc(0, 2, 0x00ffffff, 0, 32);
            }
            break;
        }
        case BNE:
        ...
        case DIVS: {
            ...
            mc = mc | i.ops[0].getReg() << 16 | i.ops[1].getReg() | i.ops[2].getReg() << 8;
            break;
        }
        case REM:
        case REMS: {
