...
void ArrayType::calcAddress(Codegen *c, Value *a, Expr *i)
{
    ...
    if (ss->is_number)
    {
        c->block()->add(Insn(MOVE, v, Operand::usigc(pointed_type->size() / 8 * ss->val)));
    }
    else
    {
...
bool StructType::construct(Codegen *c, Value *t, Value *)
{
    ...
    c->block()->add(Insn(MOVE, ra, Operand::section(IMAGE_RTTI, offs)));
...
Value *FunctionType::generateFuncall(Codegen *c, Funcall *f, Value *sl, Value *fp, std::vector<Value *> &args)
{
    Value *to_add = 0;
    ...
    c->block()->add(Insn(ADD, fp_holder, fp_holder, Operand::usigc(assembler->pointerSize() / 8)));
    c->block()->add(Insn(STORE, new_frame, Operand::reg(assembler->framePointer())));

    // Set initial stored ip to start of function
    c->block()->add(Insn(STORE, new_frame, Operand::sigc(assembler->ipOffset()), fp_holder));

    if (sl)
    {
        c->block()->add(Insn(STORE, new_frame, Operand::sigc(assembler->staticLinkOffset()), sl));
    }
...
void ActivationType::activate(Codegen *c, Value *frame)
{
    ...
    c->block()->add(
        Insn(STORE, Operand::reg(assembler->framePointer()), Operand::sigc(assembler->ipOffset()), ip_holder));

    RegSet res;
    res.set(0);
    c->block()->setReservedRegs(res);
    c->block()->add(Insn(LOAD, Operand::reg(0), frame, Operand::sigc(assembler->ipOffset())));
    c->block()->add(Insn(MOVE, Operand::reg(assembler->framePointer()), frame));
...
Value *FunctionType::allocStackFrame(Codegen *c, Value *faddr, Value *&to_add, Funcall *f, Type *rettype)
{
...
bool FunctionType::validArgList(std::vector<Value *> &args, std::string &reason)
{
    ...
    for (unsigned int loopc = 0; loopc < args.size(); loopc++)
    {
        if (args[loopc]->type->size() != params[loopc].type->size())
        {
            ...
            {
                char buf[4096];
                sprintf(buf, "%s - expected %d bit %s argument, got %d bit %s", params[loopc].name.c_str(),
                        params[loopc].type->size(), params[loopc].type->isSigned() ? "signed" : "unsigned",
                        args[loopc]->type->size(), args[loopc]->type->isSigned() ? "signed" : "unsigned");
                reason = buf;
                return false;
            }
        }
...
Value *ExternalFunctionType::generateFuncall(Codegen *c, Funcall *f, Value *, Value *fp, std::vector<Value *> &args)
{
...
{
    return (a->getType()->getSignature()) > (b->getType()->getSignature());
}
...
Value *GenericFunctionType::generateFuncall(Codegen *c, Funcall *f, Value *sl, Value *fp, std::vector<Value *> &args)
{
    ...
    for (unsigned int loopc = 0; loopc < args.size(); loopc++)
    {
        ...
        possible_matches_header->add(Insn(ADD, pointer, pointer, Operand::usigc(wordsize)));
        ...
        possible_matches_body->add(Insn(ADD, pointer, pointer, Operand::usigc(wordsize)));
        ...
        possible_matches_body->add(Insn(SUB, possible_matches, possible_matches, Operand::usigc(1)));
...
void Mtables::createSection(Image *i, Assembler *a)
{
    ...
    for (auto &me: data)
    {
        ...
        if (sf_bit)
        {
            ...
            if (me.target)
            {
                FunctionTableRelocation *mr = new FunctionTableRelocation(i, me.target, ptr - orig, IMAGE_MTABLES);
                mr->add64();
#ifdef DEBUG_MTABLES
                printf(" (%llx)ptr\n", ptr - orig);
#endif
